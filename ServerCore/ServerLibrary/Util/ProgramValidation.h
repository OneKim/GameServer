...
class ProgramValidation
{
	...
	class ProgramExpire
	{
		...
		{
			//문자열 시간을 tick_t화 하기
			//https://msdn.microsoft.com/en-us/library/ta5wcy3s.aspc
			locale loc;
			...
			if (st & ios_base::failbit) {
				cout << "time_get::get_time(" << birthDate.rdbuf()->str()
						<< ") FAILED on char: " << *i << endl;
				ASSERT(FALSE);
			}
			array<WCHAR, SIZE_64> dataTime;
			snwprintf(dataTime, L"%4d-%2d-%2d %d:%d:%d",
								t.tm_year + 1900, t.tm_mon + 1, t.tm_mday, 0, 0, 0);
